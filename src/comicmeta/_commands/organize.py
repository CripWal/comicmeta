...
def _folder_issues(source: Path, path: Path, folder_parts: tuple[str, str] | None = None) -> list[str]:
    ...
    # Loose file directly under a publisher root.
    if len(parts) == 1 and parts[0] in PUBLISHER_ROOTS:
        issues.append(f"loose file at publisher root `{parts[0]}`; needs a series folder")
        return issues
    # Umbrella wrapper: series not directly under a publisher root.
    if len(parts) > 2:
        issues.append(f"umbrella wrapper '{'/'.join(parts[:-1])}' before series folder")
...
def run(args: argparse.Namespace) -> None:
    ...
    # Canonical folder names keyed by the existing folder path.
    ...
    structural = []
    for path in archives:
        for issue in _folder_issues(source, path, folder_parts[path.parent]):
            structural.append((path.relative_to(source), issue))

    log_lines = []
    applied = skipped = 0

    print(f"  PLAN folders={len(folder_moves)} files={len(file_renames)} manual={len(structural)}")
    print()
    ...
    print(f"  ORGANIZE folders={len(folder_moves)} files={len(file_renames)} "
          f"manual={len(structural)} applied={applied} skipped={skipped}")
    if dry:
        if folder_moves or file_renames:
